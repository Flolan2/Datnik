...
from sklearn.model_selection import (
    ...
    train_test_split, StratifiedKFold # <<< ADDED train_test_split, StratifiedKFold
)
...
log_format = '%(asctime)s - %(levelname)s - [%(module)s:%(funcName)s:%(lineno)d] - %(message)s' # More detailed format
...
logger.info(f"Comparing splitting modes: {config.SPLITTING_MODES_TO_RUN}")
...
# <<< MODIFIED: Add a level for the splitting mode >>>
...
all_runs_meta_importances = collections.defaultdict(lambda: collections.defaultdict(dict)) # Mode -> MetaKey -> List

# --- Main Experiment Loop ---
# <<< ADDED: Outer loop for splitting mode >>>
for split_mode in config.SPLITTING_MODES_TO_RUN:
    ...
    for i_rep in range(config.N_REPETITIONS):
        ...
        # --- 1. Perform Train/Test Split based on Mode ---
        X_train_val, X_test, y_train_val, y_test, groups_train_val = None, None, None, None, None
        inner_cv = None
        use_groups_for_cv = False

        try:
            if split_mode == 'group':
                use_groups_for_cv = True
                ...
                use_groups_for_cv = False
                logger.info(f"  Performing STANDARD Stratified split (ignoring groups)...")
                X_full_reset = X_full.reset_index(drop=True); y_full_reset = y_full.reset_index(drop=True)
                try:
                    X_train_val, X_test, y_train_val, y_test = train_test_split(
                        X_full_reset, y_full_reset, test_size=config.TEST_SET_SIZE,
                        random_state=current_random_state, stratify=y_full_reset
                    )
                    groups_train_val = None # Not used
                except ValueError as e_split:
                    logger.error(f"Rep {i_rep+1} ({split_mode}): Error during standard train_test_split: {e_split}")
                    continue

                min_samples_per_class = y_train_val.value_counts().min() if not y_train_val.empty else 0
                actual_n_splits_standard = min(config.N_SPLITS_CV, min_samples_per_class if min_samples_per_class is not None else 0)
                if actual_n_splits_standard >= 2:
                    inner_cv = StratifiedKFold(n_splits=actual_n_splits_standard, shuffle=True, random_state=current_random_state)
                else:
                    logger.warning(f"Rep {i_rep+1} ({split_mode}): Too few samples per class ({min_samples_per_class}) for inner {actual_n_splits_standard}-fold CV. OOF/Tuning might fail.")
                    inner_cv = None
            else:
                logger.error(f"Unknown split_mode: {split_mode}. Skipping repetition.")
                continue

            logger.info(f"  Mode '{split_mode}' Split: {len(X_train_val)} train rows | {len(X_test)} test rows")

            if X_train_val is None or X_train_val.empty or X_test is None or X_test.empty:
                 logger.warning(f"  Rep {i_rep+1} ({split_mode}): Split resulted in empty train or test set. Skipping.")
                 continue
            train_classes = y_train_val.unique(); test_classes = y_test.unique()
            if len(train_classes) < 2 or len(test_classes) < 2:
                 logger.warning(f"  Rep {i_rep+1} ({split_mode}): Split resulted in single class in train ({len(train_classes)}) or test ({len(test_classes)}). Skipping.")
                 continue
        ...
        for exp_config in config.CONFIGURATIONS_TO_RUN:
            ...
            for task_prefix in config.TASKS_TO_RUN_SEPARATELY:
                ...
                try:
                    pipeline, param_dist = pipeline_builder.build_pipeline_from_config(
                ...
                try:
                    if inner_cv is None:
                         ...
                         if config.ENABLE_TUNING and param_dist:
                             logger.info(f"      [{task_prefix}] Tuning hyperparameters...")
                             random_search = RandomizedSearchCV(
                                 estimator=pipeline, param_distributions=param_dist, n_iter=config.N_ITER_RANDOM_SEARCH,
                             ...
                             random_search.fit(X_train_val_task, y_train_val, **fit_params)
                             ...
                             logger.info(f"      [{task_prefix}] Fitting model...")
                             best_pipeline = clone(pipeline)
                             best_pipeline.fit(X_train_val_task, y_train_val)
                             fit_successful = True
                    ...
                    if fit_successful:
                        ...
                        all_runs_metrics[split_mode][config_name][task_prefix].append(test_metrics) # Store under mode
                        if test_roc_data: all_runs_roc_data[split_mode][config_name][task_prefix].append(test_roc_data) # Store under mode
                        ...
                        if imp is not None: all_runs_base_importances[split_mode][config_name][task_prefix].append(imp) # Store under mode
                    ...
                    all_runs_metrics[split_mode][config_name][task_prefix].append({}) # Store under mode
                    repetition_oof_preds[config_name][task_prefix] = None
                 ...
                 if len(valid_stacking_tasks) == len(config.STACKING_TASKS_TO_COMBINE):
                     ...
                     if meta_train_features_df.isnull().any().any() or meta_test_features_df.isnull().any().any():
                          ...
                          try:
                              ...
                              all_runs_metrics[split_mode][stacking_results_key]['meta'].append(stacked_metrics) # Store under mode
                              if stacked_roc_data: all_runs_roc_data[split_mode][stacking_results_key]['meta'].append(stacked_roc_data) # Store under mode
                              ...
                              if meta_imp is not None:
                                   ...
                                   all_runs_meta_importances[split_mode][meta_storage_key].append(meta_imp) # Store under mode
...
logger.info("\n--- Aggregating Results (Across Modes) ---")
# IMPORTANT: Assumes results_processor functions have been updated to handle the 'mode' level
...
agg_base_importances = results_processor.aggregate_importances(
    all_runs_base_importances, file_prefix="base_model_importance"
)

logger.info("Aggregating Meta Importances...")
# Format meta importances to match expected 3-level structure for aggregation function
...
for mode, mode_data in all_runs_meta_importances.items():
    for meta_key, imp_list in mode_data.items():
        formatted_meta_importances[mode][meta_key]['meta_coeffs'] = imp_list # Use 'meta_coeffs' as task key
agg_meta_importances = results_processor.aggregate_importances(
    formatted_meta_importances, file_prefix="meta_model_importance"
)


# --- 5. Generate Plots ---
# IMPORTANT: Assumes plotting functions have been updated to handle 'mode' level/column
if config.GENERATE_PLOTS:
    logger.info("\n--- Generating Plots (Comparing Modes) ---")
        ...
        if plot_summary_df.empty:
            ...
            for metric in ['roc_auc', 'accuracy', 'f1_macro']:
                 if f"{metric}_mean" in plot_summary_df.columns:
                     ...
                     fname = os.path.join(config.PLOT_OUTPUT_FOLDER, f"plot_agg_metric_{metric}_comparison_modes.png")
                     try:
                         if hasattr(plotting, 'plot_metric_distributions'):
                             # Pass df which should now have 'Mode'. Plotting fn needs 'hue'/'col' logic.
                             plotting.plot_metric_distributions(plot_summary_df, metric, plot_title, fname)
            ...
            # --- Plot ROC ---
            # Combined ROC plots need significant updates in plotting.py
            # Generating separate plots per mode as a simpler alternative:
            logger.info("Generating separate ROC plots per mode (combined plot requires plotting.py update).")
            for mode in config.SPLITTING_MODES_TO_RUN:
                mode_roc_data = all_runs_roc_data.get(mode)
                mode_metrics = all_runs_metrics.get(mode)
                if not mode_roc_data or not mode_metrics:
                    logger.warning(f"No ROC data/metrics found for mode '{mode}', skipping ROC plot.")
                    continue

                # Identify valid configs/tasks within this mode that have data
                ...
                for idx, row in mode_plot_summary.iterrows():
                     ...
                     # Check if data actually exists for this combo in this mode
                ...
                if configs_tasks_for_mode_roc:
                     ...
                     try:
                          if hasattr(plotting, 'plot_aggregated_roc_curves'):
                              # Pass only data for the current mode
                     ...
                     logger.warning(f"No valid ROC data found to plot for mode '{mode}'.")


            # --- Plot Importances ---
            # Assumes aggregate_importances now returns keys like (mode, config, task) or similar structure
            # Needs updates based on actual return value of modified results_processor.aggregate_importances
            logger.info("Generating separate importance plots per mode (combined plot requires updates).")
            all_aggregated_imps_structured = {} # Reconstruct based on assumed output format
            ...
            for mode, mode_data in agg_meta_importances.items():
                 for cfg, tasks in mode_data.items():
                     for task, df in tasks.items(): all_aggregated_imps_structured[(mode, cfg, task)] = df

            if not all_aggregated_imps_structured:
                 ...
                 for (mode, config_name_imp, task_name_imp), imp_df in all_aggregated_imps_structured.items():
                      if imp_df is not None and not imp_df.empty:
                           try:
                               ...
                               is_meta = task_name_imp == 'meta_coeffs' # Check the artificial task name
